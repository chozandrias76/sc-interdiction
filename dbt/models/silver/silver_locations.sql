...
with recursive location_tree as (
    ...
    where parent_id is null
       or parent_id not in (select id from {{ ref('stg_locations') }})
...
lagrange_planet_ids as (
    select
        ...
        lp.system_name,
        stg.id as planet_id
    from {{ ref('lagrange_planets') }} lp
    inner join {{ ref('stg_locations') }} stg on stg.name = lp.planet_loc_name
)
...
left join {{ ref('display_names') }} dn on lt.name = dn.loc_name
