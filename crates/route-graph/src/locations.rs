...
pub static LOCATION_POSITIONS: LazyLock<HashMap<&'static str, LocationPosition>> =
    LazyLock::new(|| {
        ...
        add_location(&mut map, "lorville", "Stanton", hurston_pos, Some("Hurston"));
        add_location(&mut map, "everus harbor", "Stanton", moon_pos(hurston_pos, 0.05, 90.0), Some("Hurston"));

        // Hurston moons (spread around Hurston at different angles)
        add_location(&mut map, "arial", "Stanton", moon_pos(hurston_pos, 0.3, 30.0), Some("Hurston"));
        add_location(&mut map, "aberdeen", "Stanton", moon_pos(hurston_pos, 0.35, 120.0), Some("Hurston"));
        add_location(&mut map, "magda", "Stanton", moon_pos(hurston_pos, 0.28, 210.0), Some("Hurston"));
        add_location(&mut map, "ita", "Stanton", moon_pos(hurston_pos, 0.32, 300.0), Some("Hurston"));

        // Crusader landing zones & stations
        add_location(&mut map, "orison", "Stanton", crusader_pos, Some("Crusader"));
        add_location(&mut map, "port olisar", "Stanton", moon_pos(crusader_pos, 0.05, 90.0), Some("Crusader"));
        add_location(&mut map, "seraphim", "Stanton", moon_pos(crusader_pos, 0.07, 100.0), Some("Crusader"));

        // Crusader moons (spread around Crusader)
        add_location(&mut map, "cellin", "Stanton", moon_pos(crusader_pos, 0.4, 0.0), Some("Crusader"));
        add_location(&mut map, "daymar", "Stanton", moon_pos(crusader_pos, 0.45, 120.0), Some("Crusader"));
        add_location(&mut map, "yela", "Stanton", moon_pos(crusader_pos, 0.38, 240.0), Some("Crusader"));
        ...
        add_location(&mut map, "baijini point", "Stanton", moon_pos(arccorp_pos, 0.05, 90.0), Some("ArcCorp"));

        // ArcCorp moons
        add_location(&mut map, "lyria", "Stanton", moon_pos(arccorp_pos, 0.25, 60.0), Some("ArcCorp"));
        add_location(&mut map, "wala", "Stanton", moon_pos(arccorp_pos, 0.3, 180.0), Some("ArcCorp"));

        // microTech landing zones & stations
        add_location(&mut map, "new babbage", "Stanton", microtech_pos, Some("microTech"));
        add_location(&mut map, "port tressler", "Stanton", moon_pos(microtech_pos, 0.05, 90.0), Some("microTech"));

        // microTech moons
        add_location(&mut map, "calliope", "Stanton", moon_pos(microtech_pos, 0.35, 30.0), Some("microTech"));
        add_location(&mut map, "clio", "Stanton", moon_pos(microtech_pos, 0.4, 150.0), Some("microTech"));
        add_location(&mut map, "euterpe", "Stanton", moon_pos(microtech_pos, 0.32, 270.0), Some("microTech"));
        ...
        add_location(&mut map, "hur-l3", "Stanton", orbital_pos(12.85, 225.0), None); // opposite side
        add_location(&mut map, "hur-l4", "Stanton", orbital_pos(12.85, 105.0), None); // 60° ahead
        add_location(&mut map, "hur-l5", "Stanton", orbital_pos(12.85, -15.0), None); // 60° behind

        // CRU-L stations (Crusader is at 135 degrees)
        add_location(&mut map, "cru-l1", "Stanton", orbital_pos(15.0, 135.0), None);
        add_location(&mut map, "cru-l4", "Stanton", orbital_pos(18.96, 195.0), None); // 60° ahead
        add_location(&mut map, "cru-l5", "Stanton", orbital_pos(18.96, 75.0), None);  // 60° behind

        // ARC-L stations (ArcCorp is at 225 degrees)
        add_location(&mut map, "arc-l1", "Stanton", orbital_pos(15.0, 225.0), None);

        // MIC-L stations (microTech is at 315 degrees)
        add_location(&mut map, "mic-l1", "Stanton", orbital_pos(18.0, 315.0), None);
        add_location(&mut map, "mic-l2", "Stanton", orbital_pos(27.0, 315.0), None);

        // Grim HEX (near Yela)
        let yela_pos = moon_pos(crusader_pos, 0.38, 240.0);
        add_location(&mut map, "grim hex", "Stanton", moon_pos(yela_pos, 0.1, 45.0), Some("Yela"));
        ...
        // Pyro stations
        add_location(&mut map, "ruin station", "Pyro", moon_pos(pyro1_pos, 0.5, 90.0), Some("Pyro I"));
        add_location(&mut map, "checkmate station", "Pyro", moon_pos(pyro4_pos, 0.5, 90.0), Some("Pyro IV"));
...
pub fn estimate_position(location: &str) -> Option<Point3D> {
    ...
    if let Some(loc) = LOCATION_POSITIONS.get(loc_lower.as_str()) {
        return Some(loc.position.clone());
    }
...
mod tests {
    ...
    fn test_distance_calculation() {
        // Hurston to Crusader should be roughly 31 million km
